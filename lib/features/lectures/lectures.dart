...
class _LecturesScreenState extends State<LecturesScreen> {
  ...
  StaffLecturesTab _staffTab = StaffLecturesTab.manage;
    ...
    final body = Column(
      ...
      children: [
        Padding(
          ...
          child: StaffLecturesTabBar(
            value: _staffTab,
            onChanged: (value) => setState(() => _staffTab = value),
          ),
        ),
        SizedBox(height: responsive.spacing(12)),
        Expanded(
          child: _staffTab == StaffLecturesTab.monitor
              ? const LiveMonitoringView()
              : _buildManagePanel(data, auth, subjects),
        ),
      ],
    );

    if (!widget.showAppBar) {
      return ShellTabScaffold(body: body);
    }
  ...
  Widget _buildManagePanel(
    ...
    return RefreshIndicator(
      onRefresh: () => AppRefresh.reload(context),
  ...
  Widget _buildStudentView(ApiDataService data, AuthService auth) {
    ...
    if (_studentSelectedSubject != null) {
      ...
      final content = StudentLectureList(
        subject: subject,
        lectures: data.getLecturesForStudentData(
          subjects: [subject],
          section: section,
        ),
      ...
      return Scaffold(
        ...
        appBar: AppTopBar(
          title: subject,
          onBackTap: () => setState(() => _studentSelectedSubject = null),
          height: AppTopBar.heightOf(context),
        ),
        body: content,
      );
    ...
    final responsive = ResponsiveLayout.of(context);
    final hPad = responsive.horizontalPadding;
    final totalLectures = counts.values.fold<int>(0, (sum, v) => sum + v);
        ...
        Expanded(
          child: subjects.isEmpty
              ? const _EmptyLecturesState(
                  ...
                  hint: 'تواصل مع الإدارة لإضافة موادك الدراسية',
                )
              : StudentSubjectGrid(
                  subjects: subjects,
            ...
            body: SafeArea(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  final responsive = ResponsiveLayout.of(context);
                  final player = PremiumVideoPlayer(
                  ...
                  final details = _LectureDetailContent(
                    lecture: lecture,
                    averageRating: rating.avg,
                    ratingCount: rating.count,
                  );

                  // On a wide canvas the video and the discussion sit side by
                  // side, which is what makes the screen feel built for a
                  // tablet: the player stays large and the comments stay
                  // readable, instead of the player eating the whole fold.
                  if (responsive.isWideCanvas && constraints.maxWidth >= 860) {
                    final gutter = responsive.horizontalPadding;
                    return Padding(
                      padding: EdgeInsets.fromLTRB(
                        gutter,
                        responsive.spacing(AppLayout.pageTop),
                        gutter,
                        0,
                      ),
                      child: Row(
                  ...
                  // Stacked layout. The 16:9 player is capped at a share of
                  // the visible height so a landscape phone does not push the
                  // rest of the page off the bottom of the column.
                  final gutter =
                      responsive.horizontalPadding +
                      responsive.centeringInset();
                  final maxPlayerHeight = constraints.maxHeight * 0.52;
                      ...
                      Padding(
                        ...
                        child: ConstrainedBox(
                          constraints: BoxConstraints(
                            maxHeight: maxPlayerHeight.clamp(
                              160.0,
                              double.infinity,
                            ),
                          ),
                        ...
                        child: CustomScrollView(
                          slivers: [
                            SliverPadding(
                              padding: EdgeInsets.fromLTRB(
                                gutter,
                                responsive.spacing(AppLayout.blockGap),
                                gutter,
                                responsive.spacing(AppLayout.pageBottom),
                              ),
                              sliver: SliverToBoxAdapter(child: details),
                            ),
                          ],
...
/// Everything below (or beside) the player on the lecture screen.
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(height: responsive.spacing(AppLayout.sectionGap)),
        LectureCommentsSection(lectureId: lecture.id),
      ],
    );
  }
}
