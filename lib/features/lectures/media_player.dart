...
class _FullscreenPageState extends State<_FullscreenPage> {
  ...
  void didChangeDependencies() {
    ...
    // Phones get rotated into landscape because a 16:9 video in portrait
    // wastes most of the screen. Tablets are left alone: an iPad is perfectly
    // usable in portrait, and Apple expects iPad apps to honour every
    // orientation rather than seize one.
    if (!ResponsiveLayout.of(context).isTablet) {
  ...
  void dispose() {
    ...
    // Hand orientation back to the platform manifest rather than pinning the
    // app to portrait — pinning here used to outlive the player and lock the
    // entire session, including on iPad.
    if (_forcedLandscape) {
      SystemChrome.setPreferredOrientations(const []);
    }
    super.dispose();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // The player paints its own safe-area-aware controls, so the stage is
      // free to fill the display corner to corner behind them.
...
class _ControlsOverlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = host.controller!;
    // In fullscreen the stage runs edge to edge, so the controls carry the
    // whole inset themselves: the notch/Dynamic Island (which moves to a side
    // in landscape), the rounded corners, and the home indicator.
