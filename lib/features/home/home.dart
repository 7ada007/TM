...
class AppShellScaffold extends StatefulWidget {
  ...
  /// Accessibility names for each tab. These are announced to screen readers
  /// and used for the back-button semantics; they are deliberately *not*
  /// painted in the top bar, where the bottom navigation already names the
  /// active tab.
...
class _AppShellScaffoldState extends State<AppShellScaffold> {
  ...
  /// Whether the active tab has scrolled far enough for the top bar to need a
  /// surface behind it. Tracked per tab so switching tabs restores the right
  /// state rather than inheriting the previous tab's.
  final Set<int> _scrolledTabs = <int>{};

  bool get _barElevated => _scrolledTabs.contains(_currentIndex);
  ...
  /// Raises the bar once content passes under it.
  ///
  /// Only vertical scrollables count: the horizontal subject rail on the home
  /// tab also emits notifications, and letting those through would flicker the
  /// hairline on and off as the user swipes sideways.
  bool _onScroll(ScrollNotification notification) {
    if (notification.metrics.axis != Axis.vertical) return false;

    const threshold = 4.0;
    final scrolled = notification.metrics.pixels > threshold;
    if (scrolled == _scrolledTabs.contains(_currentIndex)) return false;

    setState(() {
      if (scrolled) {
        _scrolledTabs.add(_currentIndex);
      } else {
        _scrolledTabs.remove(_currentIndex);
      }
    });
    return false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ShellTabs(
      ...
      child: AssistantDraggableFab(
        // The system back gesture closes the slide-in menu before it is allowed
        // to leave the shell, so back never skips a visible layer.
        child: PopScope(
          canPop: !_menuOpen,
          ...
          child: Scaffold(
            ...
            appBar: AppTopBar(
              ...
              menuOpen: _menuOpen,
              // Flat over a resting page, surfaced once content slides under.
              elevated: _barElevated || _menuOpen,
            ),
            body: Semantics(
              // Keeps the active tab discoverable to assistive tech now that
              // the title is no longer painted in the bar.
              label: widget.tabTitles[_currentIndex],
              container: true,
              child: Stack(
                fit: StackFit.expand,
                children: [
                  // Page content clears landscape notches and rounded corners…
                  Padding(
                  ...
                  // …while the menu sits above that inset so its scrim covers
                  // the full width, with no unscrimmed strip along the notch.
...
class PremiumBottomNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxWidth = responsive.bottomNavMaxWidth;
    ...
    return RepaintBoundary(
      child: Padding(
        // Left/right pick up the landscape notch inset; the bottom clears the
        // home indicator on gesture-navigation hardware and collapses to the
        // plain gap on devices with physical buttons.
        padding: EdgeInsets.fromLTRB(
          responsive.horizontalPadding + responsive.padding.left,
          responsive.spacing(4),
          responsive.horizontalPadding + responsive.padding.right,
...
/// Horizontally scrolling shortcut rail of the signed-in student's subjects.
///
/// Tile and rail heights are derived rather than fixed so the label still fits
/// at the largest supported text scale, and the tiles grow on a tablet instead
/// of leaving a band of empty card.
class _SubjectRail extends StatelessWidget {
  final List<String> subjects;

  const _SubjectRail({required this.subjects});

  @override
  Widget build(BuildContext context) {
    final responsive = ResponsiveLayout.of(context);
    final iconSize = responsive.spacing(44).clamp(40.0, 60.0);
    final labelSize = responsive.fontSize(10.5);
    final tileWidth = responsive.spacing(96).clamp(88.0, 128.0);
    final railHeight =
        iconSize +
        responsive.spacing(8) +
        labelSize * responsive.textScale * 1.6 +
        28;
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        separatorBuilder: (_, _) =>
            SizedBox(width: responsive.spacing(AppLayout.itemGap)),
        itemBuilder: (context, index) {
          ...
          ).animate().fadeIn(delay: (140 + 40 * index).ms, duration: 280.ms);
        },
      ),
    );
  }
...
class StudentHomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final classmatesCount = PermissionUtils.visibleStudentsFor(
      user,
      data.students,
    ).where((s) => s.id != user.id).length;
          ...
          sliver: SliverList(
            delegate: SliverChildListDelegate([
              ...
              if (subjects.isNotEmpty) ...[
                ...
                _SubjectRail(subjects: subjects),
                const SizedBox(height: AppLayout.sectionGap),
              ],
              if (posts.isNotEmpty) ...[
                HomeSectionTitle(
  ...
  Widget build(BuildContext context) {
    return AppShellScaffold(
      ...
      pages: const [
        ...
        StudentDirectoryScreen(showAppBar: false, title: 'الزملاء'),
