...
class _StudentDirectoryScreenState extends State<StudentDirectoryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Controls sit inside the same centring inset the grid below uses, so the
    // search field and the first column of cards share one left edge on iPad.
    final hPad =
        responsive.horizontalPadding +
        responsive.centeringInset(maxWidth: responsive.maxWideContentWidth);
...
class _DirectoryGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final padding = responsive.listPadding(wide: true);
    final gap = responsive.spacing(12);
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // Card height tracks the text scale so a large-type user does not get
        // a clipped name, and grows a little on tablets to match the wider
        // tile and larger avatar.
        final extent =
            responsive.spacing(196) + (responsive.textScale - 1) * 56;
...
class _StudentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avatarSize = responsive.spacing(76).clamp(68.0, 96.0);
...
class _DirectoryEmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: responsive.listPadding(wide: true),
      physics: const AlwaysScrollableScrollPhysics(),
      children: [
