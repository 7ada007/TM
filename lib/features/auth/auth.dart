...
abstract final class PremiumPageTransitions {
  ...
  /// Transition used by `/splash` only.
  ///
  /// The splash is the first route, so its *forward* transition never runs —
  /// Navigator shows the initial page already settled. Only the reverse leg
  /// matters, and it is the half of the hand-off to `/login` that the splash
  /// itself cannot control.
  ///
  /// The reverse duration matches [standard] (the duration `/login` fades in
  /// over) and the curve is the exact complement of the `easeOut` that
  /// `_fadeSlideUpImpl` fades in with: `flipped` gives `1 - easeOut(1 - t)`,
  /// so at every instant `splashOpacity + loginOpacity == 1`. The two layers
  /// therefore never both go translucent at once, and the app backdrop
  /// underneath is never revealed — no flash, no black frame, no seam.
  static CustomTransitionPage<T> splashHandoff<T>({
    required LocalKey key,
    required Widget child,
  }) {
    return CustomTransitionPage<T>(
...
class _LoginScreenState extends State<LoginScreen>
  ...
  Widget build(BuildContext context) {
    ...
    // Shrink the mark on short screens (iPhone SE in portrait, any phone in
    // landscape) so the form stays reachable without scrolling.
    final logoHeight = responsive.size.height < 700
        ? 72.0
        : (responsive.isTablet ? 112.0 : 96.0);
                ...
                child: ConstrainedBox(
                  ...
                  child: Padding(
                    padding: EdgeInsets.fromLTRB(
                      responsive.horizontalPadding + 6,
                      6,
                      responsive.horizontalPadding + 6,
                      24,
                    ),
                    // A sign-in form has no reason to be 1000pt wide on an
                    // iPad; cap it and centre it so it reads as a considered
                    // card rather than a stretched phone screen.
...
/// Text field used on the sign-in screen.
///
/// Owns its own chrome rather than leaning on `InputDecoration` so the focus
/// ring, the fill, the error state and the label can all move together on one
/// curve. Built on [FormField] so it still participates in the surrounding
/// [Form] — validation, save and reset all behave exactly as they would with a
/// plain `TextFormField`.
...
class _AuthTextFieldState extends State<AuthTextField>
  ...
  /// Nudges the field sideways when it first goes invalid.
  ///
  /// Fired only on the null → error transition so re-validating an
  /// already-invalid field on every keystroke does not rattle continuously.
  void _reactToError(String? error) {
    if (error != null && _lastError == null && !prefersReducedMotion(context)) {
    ...
    return FormField<String>(
      ...
      builder: (field) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) _reactToError(error);
        });
        ...
        return AnimatedBuilder(
          animation: _shake,
          builder: (context, child) {
            // Three decaying oscillations; amplitude falls to zero so the
            // field settles exactly where it started.
            ...
            children: [
              ...
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  // A soft halo instead of a heavier border, so gaining focus
                  // reads as the field lifting rather than thickening.
                  boxShadow: _focused && !hasError
                      ...
                      : AppShadows.of(Theme.of(context).brightness),
                ),
                child: Row(
                  children: [
                    Padding(
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        onChanged: (value) {
                          field.didChange(value);
                          // Clear a stale error as soon as the user starts
                          // fixing it, rather than making them submit again.
              ...
              // Reserves no space until there is something to say, so the form
              // does not carry permanent gaps under every field.
              MotionSize(
                child: hasError
                    ? Padding(
                        padding: const EdgeInsetsDirectional.only(
        ...
        child: Column(
          ...
          children: [
            ...
            AuthTextField(
              ...
              onSubmitted: isBusy ? null : onLogin,
              validator: (value) {
            ...
            Align(
              ...
              child: TextButton(
                ...
                style: TextButton.styleFrom(
                  // Padding rather than a shrunk tap target: the label is
                  // small, so the touch area has to come from somewhere.
...
class _StarFieldPainter extends CustomPainter {
  ...
  static List<Offset> _generate() {
    ...
    return List.generate(46, (_) => Offset(rnd.nextDouble(), rnd.nextDouble()));
  }
...
class _GlobePainter extends CustomPainter {
  ...
  void _drawConnections(Canvas canvas, Offset center, double r) {
    ...
    final arcColor = isDark ? const Color(0xFF6FC1F8) : const Color(0xFF3B82F6);
