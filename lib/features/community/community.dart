...
class _CommunityScreenState extends State<CommunityScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final content = Stack(
      children: [
        ...
        // Directional so the compose button lands on the same side as every
        // other floating action in this RTL app, instead of mirroring away
        // from them.
        PositionedDirectional(
          bottom: responsive.spacing(AppLayout.pageBottom),
    ...
    return Scaffold(
      ...
      appBar: AppTopBar(title: 'المجتمع', height: AppTopBar.heightOf(context)),
      body: content,
    );
  }
  ...
  Widget _buildFeed(BuildContext context, List<CommunityPostModel> posts) {
    final responsive = ResponsiveLayout.of(context);
    final listPadding = responsive.listPadding(hasFab: true);
    ...
    return CustomScrollView(
      ...
      slivers: [
        ...
        SliverPadding(
          // A feed is a reading column: on a tablet it is centred at a
          // comfortable measure rather than run edge to edge.
          padding: listPadding.copyWith(
            top: responsive.spacing(AppLayout.sectionGap),
          ),
          sliver: SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) => Padding(
                padding: EdgeInsets.only(
                  bottom: responsive.spacing(AppLayout.itemGap),
                ),
...
class _CommunityBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sideMargin =
        ...
        responsive.centeringInset(maxWidth: responsive.maxContentWidth);
    ...
    return Container(
      ...
      // Scales with the device but stays short enough to leave the first post
      // visible on a landscape phone.
      height: responsive.spacing(200).clamp(170.0, 250.0),
      decoration: BoxDecoration(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Spacer(),
                  // Text.rich rather than RichText: RichText opts out of the
                  // ambient text scaler, which would leave this the one
                  // headline in the app that ignores the user's font size.
                  Text.rich(
                    TextSpan(
                      children: [
...
class _CommentsSheetState extends State<_CommentsSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Container(
        // Sized against the space left over once the status bar and any open
        // keyboard are accounted for, so the header stays on screen.
        height: sheetMaxHeight(context, fraction: 0.85),
        decoration: BoxDecoration(
    ...
    return Padding(
      ...
      child: Container(
        constraints: BoxConstraints(
          maxHeight: sheetMaxHeight(context, fraction: 0.95),
        ),
