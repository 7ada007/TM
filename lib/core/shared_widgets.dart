...
class AppSearchField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: hintText,
        prefixIcon: Icon(Icons.search_rounded, color: AppColors.icon(context)),
...
class AppTopBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// Heading for a pushed screen.
  ///
  /// Left null on the tabbed shell, where the bar carries no title at all: the
  /// bottom navigation already names the tab, and each tab opens with its own
  /// content hero. An empty bar is the point — it hands the top of the screen
  /// to the content instead of spending it on chrome.
  final String? title;
  final List<Widget>? actions;
  final double? height;
  final bool menuOpen;

  /// Whether the bar paints its own surface and hairline.
  ///
  /// The shell keeps this false while the page is at rest, so the bar melts
  /// into the backdrop, and raises it once content scrolls underneath — that
  /// separation only earns its keep when there is something to separate.
  final bool elevated;
  ...
  /// Width of the identical slot reserved on each side of the bar.
  ///
  /// Sized so the *glyph* inside the leading button lands on the same vertical
  /// line as the page's content gutter, rather than the button's 44pt tap box
  /// doing so — otherwise the icon reads as outdented against every card and
  /// heading below it. Reserving the same width on the trailing side keeps a
  /// centred title optically centred no matter how many actions a screen adds.
  static double sideSlotWidth(BuildContext context) {
    const buttonSize = 44.0;
    const glyphSize = 22.0;
    final gutter = ResponsiveLayout.of(context).horizontalPadding;
    return (gutter * 2 + (buttonSize - glyphSize)).clamp(52.0, 96.0);
  }

  /// Bar height for the current device.
  ///
  /// `preferredSize` cannot read a BuildContext, so screens that want the
  /// device-correct height (taller on a tablet) pass `height:
  /// AppTopBar.heightOf(context)`. Without it the bar falls back to the phone
  /// height, which looks cramped on an iPad.
  static double heightOf(BuildContext context) =>
      ResponsiveLayout.of(context).appBarHeight;
...
class _AppTopBarState extends State<AppTopBar>
  ...
  Widget build(BuildContext context) {
    ...
    final slotWidth = AppTopBar.sideSlotWidth(context);

    // The bar's own chrome is laid out LTR so the back chevron, menu button and
    // action slot keep fixed physical positions across the app; the title text
    // inside restores the ambient (RTL) direction.
...
class SubjectTeachersSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colors = AppSubjects.gradientFor(subject);
    // 0.72 of the *available* height, not of the display: with the keyboard up
    // or in landscape, a fraction of the full screen would overflow the sheet.
    final maxHeight = sheetMaxHeight(context, fraction: 0.72);
...
/// Presents a modal sheet with the app's standard chrome.
///
/// Centralised so every sheet in the app agrees on the things that are easy to
/// get subtly wrong per call site: it stays clear of the status bar and the
/// home indicator, it is width-capped and centred on a tablet instead of
/// spanning a 12.9" display, and it uses one shared open/close curve.
Future<T?> showAppBottomSheet<T>({
  ...
  final responsive = ResponsiveLayout.of(context);
  final maxWidth = responsive.isTablet ? 640.0 : double.infinity;
  ...
  return showModalBottomSheet<T>(
    ...
    constraints: maxWidth.isFinite ? BoxConstraints(maxWidth: maxWidth) : null,
...
/// Largest height a sheet may take, as a fraction of the space genuinely
/// available to it.
///
/// Measuring against the raw screen height is the usual bug: with the keyboard
/// open, or on a landscape phone, that fraction resolves to more room than
/// exists and the sheet's header scrolls off the top of the display.
double sheetMaxHeight(BuildContext context, {double fraction = 0.75}) {
  final mq = MediaQuery.of(context);
  final available = mq.size.height - mq.padding.top - mq.viewInsets.bottom - 16;
  return (available.clamp(160.0, double.infinity)) * fraction.clamp(0.1, 1.0);
}

/// Rounded surface every bottom sheet sits on.
///
/// [showAppBottomSheet] presents sheets on a transparent background so each one
/// can round its own corners and control its own height; this supplies the fill,
/// the corner radius and the home-indicator clearance so they all match.
class AppSheetSurface extends StatelessWidget {
  final Widget child;
  final EdgeInsetsGeometry? padding;

  const AppSheetSurface({super.key, required this.child, this.padding});
...
/// Standard grab handle for the top of a sheet.
...
/// Centres and width-caps a subtree so it never stretches across a tablet.
///
/// Phones fall through untouched (the cap is infinite there), so wrapping a
/// screen in this is free on the devices that do not need it.
class ResponsiveContentFrame extends StatelessWidget {
  final Widget child;

  /// Use the wider cap intended for grids and dashboards rather than the
  /// narrower reading-column cap.
  final bool wide;

  /// Explicit override; wins over [wide].
  ...
  Widget build(BuildContext context) {
    ...
    final cap =
        maxWidth ??
        (wide ? responsive.maxWideContentWidth : responsive.maxContentWidth);

    if (!cap.isFinite) return child;
