...
class AppColors {
  ...
  /// Brand blue for the current theme.
  ///
  /// [primary] is tuned for light surfaces; on the dark surfaces it lands at
  /// roughly 2:1 contrast, which reads as muddy rather than branded. Anything
  /// drawn *on* a surface — spinner arcs, accent text, tinted glyphs — should
  /// use this instead of the raw constant.
...
/// The app's single typeface.
///
/// Readex Pro is bundled and declared in `pubspec.yaml`, so every style below
/// resolves synchronously from the asset bundle. Nothing here touches the
/// network, and nothing falls back to a system face: [family] is a constant,
/// not a value looked up from a font-loading package at runtime.
abstract final class AppFonts {
  /// The bundled family name, matching the `family:` key in pubspec.yaml.
  static const String family = 'ReadexPro';

  /// Weights actually present in the bundle. Readex Pro's variable axis tops
  /// out at 700, so anything heavier is rendered with Bold rather than being
  /// synthetically emboldened.
  static const FontWeight maxWeight = FontWeight.w700;

  /// Clamps a requested weight to what the bundled family can actually render.
  static FontWeight _resolve(FontWeight? weight) {
    if (weight == null) return FontWeight.w400;
    return weight.value > maxWeight.value ? maxWeight : weight;
  }

  static TextTheme getTextTheme(Color primary, Color secondary) {
    return TextTheme(
      displayLarge: readex(color: primary, fontWeight: FontWeight.w700),
      displayMedium: readex(color: primary, fontWeight: FontWeight.w700),
      displaySmall: readex(color: primary, fontWeight: FontWeight.w600),
      headlineLarge: readex(color: primary, fontWeight: FontWeight.w700),
      headlineMedium: readex(color: primary, fontWeight: FontWeight.w600),
      headlineSmall: readex(color: primary, fontWeight: FontWeight.w600),
      titleLarge: readex(
        color: primary,
  ...
  static TextStyle readex({
    ...
    return TextStyle(
      fontFamily: family,
      fontSize: fontSize,
      fontWeight: _resolve(fontWeight),
      color: color,
      height: height,
